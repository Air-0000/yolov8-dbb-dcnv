"""Manual training loop for DBB-DCNV4 on BCCD (bypasses YOLO.train() segfault on Blackwell)"""
...
# Set model attributes (as DetectionTrainer._setup_train would)
...
print(f'Dataset: {len(dataset)} images, {len(dataloader)} batches (batch=8)')
...
for epoch in range(EPOCHS):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        loss = loss_tuple[0].sum()  # (box+cls+dfl).sum() -> scalar
        loss.backward()
        optimizer.step()
